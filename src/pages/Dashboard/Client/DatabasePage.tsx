...
export default function DatabasePage() {
  ...
  const filteredClients = useMemo(() => {
    return clients.filter(client => {
      ...
      const matchProduct = productFilter === "Semua" || (client.fokus_produk && client.fokus_produk.includes(productFilter));
      const matchStatus = statusFilter === "Semua" || client.tipe === statusFilter;
  ...
  useEffect(() => {
      ...
      subjudul: "Daftar Jaringan Aktif",
      showBack: false,
      searchBar: (
        <div key="search-filter" className="relative flex-1 min-w-[100px] flex items-center bg-white dark:bg-[#00172D] border border-gray-400 dark:border-slate-600 rounded-full shadow-sm focus-within:ring-2 focus-within:ring-[#35ACDF]/50 h-[38px] sm:h-[40px]">
           ...
           <input
             ...
             className="pl-9 pr-10 py-1.5 sm:py-2 w-full bg-transparent focus:outline-none text-[10px] sm:text-xs font-medium rounded-full min-w-0 text-gray-800 dark:text-white"
           />
           <div className="absolute right-1 top-1/2 -translate-y-1/2 group">
             <button onClick={() => setIsFilterOpen(!isFilterOpen)} className={`w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-full transition-colors ${statusFilter !== "Semua" ? "text-[#35ACDF] bg-blue-50 dark:bg-blue-950/30" : "text-gray-400 dark:text-gray-550 hover:bg-gray-100 dark:hover:bg-white/5"}`}>
               <Filter className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
             </button>
             <div className={`absolute right-0 top-full mt-2 w-48 bg-white dark:bg-[#00172D] border border-gray-400 dark:border-slate-600 rounded-xl shadow-xl flex flex-col overflow-hidden transition-all z-[9999] ${isFilterOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2'}`}>
               <button onClick={() => { setStatusFilter("Semua"); setIsFilterOpen(false); }} className={`px-4 py-3 text-left text-xs font-bold ${statusFilter === "Semua" ? "text-[#35ACDF] bg-blue-50/50 dark:bg-blue-950/30" : "text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5"} uppercase tracking-widest border-b border-gray-50 dark:border-slate-600/5`}><Trans>Semua Jenis</Trans></button>
               <button onClick={() => { setStatusFilter("Distributor"); setIsFilterOpen(false); }} className={`px-4 py-3 text-left text-xs font-bold ${statusFilter === "Distributor" ? "text-[#35ACDF] bg-blue-50/50 dark:bg-blue-950/30" : "text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5"} uppercase tracking-widest border-b border-gray-50 dark:border-slate-600/5`}><Trans>Distributor</Trans></button>
               <button onClick={() => { setStatusFilter("Partner"); setIsFilterOpen(false); }} className={`px-4 py-3 text-left text-xs font-bold ${statusFilter === "Partner" ? "text-[#35ACDF] bg-blue-50/50 dark:bg-blue-950/30" : "text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5"} uppercase tracking-widest border-b border-gray-50 dark:border-slate-600/5`}><Trans>Partner</Trans></button>
               <button onClick={() => { setStatusFilter("Calon Mitra"); setIsFilterOpen(false); }} className={`px-4 py-3 text-left text-xs font-bold ${statusFilter === "Calon Mitra" ? "text-[#35ACDF] bg-blue-50/50 dark:bg-blue-950/30" : "text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5"} uppercase tracking-widest`}><Trans>Calon Mitra</Trans></button>
             </div>
           </div>
        </div>
      ),
        ...
        <>
          ...
          <div className="hidden md:flex items-center gap-2">
            <button 
              ...
              className="hidden md:flex w-[40px] h-[40px] md:w-auto px-0 md:px-4 bg-[#35ACDF] hover:bg-[#2c91bd] text-white text-[10px] font-bold uppercase tracking-widest rounded-full shadow-sm transition-all items-center justify-center gap-1.5 shrink-0 hover:scale-105"
            >
              ...
              {isActionMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-[#00172D] border border-gray-200 dark:border-slate-800 rounded-xl shadow-xl flex flex-col transition-all z-50">
                     ...
                     <button onClick={() => { exportData(filteredClients, 'data_mitra', 'csv'); setIsActionMenuOpen(false); }} className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 uppercase tracking-widest border-b border-gray-50 dark:border-slate-600/5 flex items-center gap-2">
                        <FileText className="w-3.5 h-3.5 text-gray-400 dark:text-gray-550" /><Trans>File CSV</Trans></button>
                     <button onClick={() => { exportData(filteredClients, 'data_mitra', 'json'); setIsActionMenuOpen(false); }} className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 uppercase tracking-widest flex items-center gap-2">
          ...
          <div className="md:hidden flex flex-col w-full min-w-[160px] -m-1 bg-white dark:bg-[#00172D]">
            ...
            <button onClick={() => { exportData(filteredClients, 'data_mitra', 'csv'); }} className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 whitespace-nowrap tracking-widest border-b border-gray-50 dark:border-slate-600/5 flex items-center gap-3">
               <FileText className="w-4 h-4 text-gray-400 dark:text-gray-550" /><Trans>File CSV</Trans></button>
            <button onClick={() => { exportData(filteredClients, 'data_mitra', 'json'); }} className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 whitespace-nowrap tracking-widest flex items-center gap-3">
               <FileText className="w-4 h-4 text-gray-400 dark:text-gray-550" /><Trans>File JSON</Trans></button>
          </div>
        </>
      )
    });
  }, [setHeaderConfig, viewMode, statusFilter, searchQuery, navigate, filteredClients, isActionMenuOpen, isFilterOpen]);
  ...
  return (
    ...
    >
      ...
      <div className="md:hidden flex px-1">
        ...
        >
          <Plus className="w-4 h-4" /><Trans>Tambah Partner</Trans></button>
      </div>
      ...
      {loading ? (
        ...
        <div className="mt-1">
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      <div className="flex flex-col">
                        {client.tipe !== "Sales" && (
                          <span className={`text-[8px] sm:text-[9px] md:text-[10px] font-black uppercase tracking-widest mb-1 ${client.tipe === "Distributor" ? "text-orange-500" : client.tipe === "Calon Mitra" ? "text-[#35ACDF]" : "text-emerald-500"}`}>
                            {client.tipe}
                          </span>
                        )}
                      </div>
                    ...
                    <h3 className="text-xs sm:text-base md:text-lg font-bold text-[#00172D] dark:text-white mb-0.5 md:mb-1 truncate">{client.perusahaan}</h3>
                    ...
                    <div className="mt-auto space-y-1.5 sm:space-y-2 pt-2 md:pt-4 border-t border-gray-50 dark:border-slate-600/5 flex-1">
                      <div className="flex items-center gap-1.5 sm:gap-2 text-[8px] sm:text-[11px] md:text-xs font-semibold text-gray-600 dark:text-gray-300">
                        <User className="w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-3.5 md:h-3.5 text-gray-400 dark:text-gray-500 shrink-0" />
                        <span className="truncate">PIC: {client.nama_penanggung_jawab}</span>
                      </div>
                      <div className="flex items-center gap-1.5 sm:gap-2 text-[8px] sm:text-[11px] md:text-xs font-semibold text-gray-600 dark:text-gray-300">
                        <Briefcase className="w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-3.5 md:h-3.5 text-[#35ACDF] shrink-0" />
                        <span className="truncate">Sales: {client.nama_penjual || "-"}</span>
                      </div>
                      <div className="flex items-center gap-1.5 sm:gap-2 text-[8px] sm:text-[11px] md:text-xs font-semibold text-gray-600 dark:text-gray-300">
                        <Phone className="w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-3.5 md:h-3.5 text-[#35ACDF] shrink-0" />
                        <span className="truncate">{client.telepon_kantor}</span>
                      </div>
                    </div>

                    <div className="mt-3 md:mt-4 flex flex-wrap gap-1.5">
                      {false && client.fokus_produk?.map(f => (
                        <span key={f} className="text-[7px] sm:text-[8px] md:text-[9px] uppercase tracking-widest font-bold bg-gray-50 dark:bg-slate-900 text-gray-500 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded">
                          {f}
                        </span>
                      ))}
                    </div>
                  ...
                  >
                    <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto">
                       ...
                       <div className="min-w-0">
                         <h3 className="font-bold text-[#00172D] dark:text-white text-base md:text-lg truncate">{client.perusahaan}</h3>
                         <p className="text-[11px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 truncate">{client.nama_penanggung_jawab} &bull; {client.provinsi}</p>
                       </div>
                    </div>
                    <div className="flex flex-col">
                        {client.tipe !== "Sales" && (
                          <span className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ${client.tipe === "Distributor" ? "text-orange-500" : client.tipe === "Calon Mitra" ? "text-[#35ACDF]" : "text-emerald-500"}`}>
                            {client.tipe}
                          </span>
                        )}
                    </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left whitespace-nowrap min-w-[600px]">
                  <thead className="bg-gray-50 dark:bg-slate-900/60 border-b border-gray-200 dark:border-slate-800">
                    <tr>
                      <th className="px-4 md:px-6 py-3 md:py-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-500 dark:text-gray-400"><Trans>Perusahaan</Trans></th>
                      <th className="px-4 md:px-6 py-3 md:py-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-500 dark:text-gray-400"><Trans>PIC / Kontak</Trans></th>
                      <th className="px-4 md:px-6 py-3 md:py-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-500 dark:text-gray-400"><Trans>Tipe / Jenis Keanggotaan</Trans></th>
                    </tr>
                  </thead>
                    ...
                    {filteredClients.map((client) => (
                      <tr key={client.id} onClick={() => navigate(`/dashboard/client/${client.id}`)} className="hover:bg-gray-50/50 dark:hover:bg-white/5 cursor-pointer transition-colors group">
                        ...
                        <td className="px-4 md:px-6 py-3 md:py-4">
                          ...
                          <div className="text-[9px] md:text-[10px] font-semibold text-gray-400 dark:text-gray-500 mt-0.5">{client.telepon_kantor}</div>
                        </td>
                        <td className="px-4 md:px-6 py-3 md:py-4">
                           {client.tipe !== "Sales" && (
                             <span className={`px-2 py-1 text-[8px] md:text-[9px] font-black uppercase tracking-widest rounded ${client.tipe === "Calon Mitra" ? "bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400" : "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400"}`}>
                               {client.tipe}
                             </span>
                           )}
                           <div className="text-[9px] md:text-[10px] font-semibold text-gray-400 dark:text-gray-500 mt-1">{client.kota}</div>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {filteredClients.length === 0 && (
            ...
            ><Trans>Tidak ada mitra yang ditemukan</Trans></motion.div>
          )}
        </div>
      )}
